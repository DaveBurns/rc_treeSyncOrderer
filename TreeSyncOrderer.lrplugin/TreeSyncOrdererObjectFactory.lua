--[[
        TreeSyncOrdererObjectFactory.lua
...
local TreeSyncOrdererObjectFactory, dbg, dbgf = ObjectFactory:newClass{ className = 'TreeSyncOrdererObjectFactory', register = false }
...
--
function TreeSyncOrdererObjectFactory:newClass( t )
...
--
function TreeSyncOrdererObjectFactory:new( t )
...
--
function TreeSyncOrdererObjectFactory:frameworkModule( spec )
...
--
function TreeSyncOrdererObjectFactory:newObject( class, ... )
    if type( class ) == 'table' then
        --if class == Manager then
        --    return TreeSyncOrdererManager:new( ... )
        --end
    elseif type( class ) == 'string' then
        ...
        elseif class == 'ExportDialog' then
            if gbl:getValue( 'ExtendedPublish' ) then -- export supported with publish service.
                ...
                return TreeSyncOrdererExport:newDialog( ... )
            end
        elseif class == 'Export' then
            if gbl:getValue( 'ExtendedPublish' ) then
                ...
                return TreeSyncOrdererExport:newExport( ... ) -- export supprted without publish service.
            end
        elseif class == 'ExportFilter' then -- note: there could be more than one export-filter defined for a single plugin, so somehow, which object to create will need to be resolved...
            ...
            --return TreeSyncOrdererExportFilter:new( ... )
        end
    end
...
return TreeSyncOrdererObjectFactory 
-- the end.