...
function ExtendedBackground:init( call )
    ...
    local ver = fprops:getPropertyForPlugin( nil, 'version' ) -- this plugin - no need to force read.
    if ver ~= 4 then
        fprops:clearPropertiesForPlugin( nil, true ) -- no-flush
        fprops:setPropertyForPlugin( nil, 'version', 4 ) -- auto-flush.
        app:log( "*** Cleared old plugin properties - going forward with version 4." )
    end
    if s then    
